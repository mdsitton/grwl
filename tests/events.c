...
static void key_callback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
    ...
    if (name)
    {
        printf("%08x to %i at %0.3f: Key 0x%04x Scancode 0x%04x (%s) (%s) (with%s) was %s\n", counter++, slot->number,
               glfwGetTime(), key, scancode, get_key_name(key), name, get_mods_name(mods), get_action_name(action));
    }
    else
    {
        printf("%08x to %i at %0.3f: Key 0x%04x Scancode 0x%04x (%s) (with%s) was %s\n", counter++, slot->number,
               glfwGetTime(), key, scancode, get_key_name(key), get_mods_name(mods), get_action_name(action));
    }
                             ...
                             int* blockSizes, int focusedBlock, int caret)
{
    ...
    printf("%08x to %i at %0.3f: Preedit text ", counter++, slot->number, glfwGetTime());
...
static void monitor_callback(GLFWmonitor* monitor, int event)
{
    if (event == GLFW_CONNECTED)
    {
        ...
        printf("%08x at %0.3f: Monitor %s (%ix%i at %ix%i, %ix%i mm) was connected\n", counter++, glfwGetTime(),
               glfwGetMonitorName(monitor), mode->width, mode->height, x, y, widthMM, heightMM);
    }
...
static void joystick_callback(int jid, int event)
{
    if (event == GLFW_CONNECTED)
    {
        ...
        printf("%08x at %0.3f: Joystick %i (%s) was connected with %i axes, %i buttons, and %i hats\n", counter++,
               glfwGetTime(), jid, glfwGetJoystickName(jid), axisCount, buttonCount, hatCount);

        if (glfwJoystickIsGamepad(jid))
        {
            printf("  Joystick %i (%s) has a gamepad mapping (%s)\n", jid, glfwGetJoystickGUID(jid),
...
{
    int i = 0;
    printf("%08x at %0.3f: Gamepad %i (%s) state:", counter++, glfwGetTime(), jid, glfwGetJoystickName(jid));
