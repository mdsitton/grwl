...
//========================================================================
//
// For font handling, I referred to https://github.com/Immediate-Mode-UI/Nuklear/wiki/Complete-font-guide.
// For nuklear handling, I referred to tests/window.c.
//
...
static nk_rune rangesJapan[] = { 0x0020, 0x007E, // Basic Latin
...
static int set_font_selecter(GLFWwindow* window, struct nk_context* nk, int height, int fontHeight)
{
    ...
    newSelectedIndex =
        nk_combo(nk, (const char**)fontFamilyNames, fontNum, currentFontIndex, fontHeight, nk_vec2(300, 400));
...
static void set_preedit_cursor_edit(GLFWwindow* window, struct nk_context* nk, int height, int* isAutoUpdating)
{
    ...
    events = nk_edit_string_zero_terminated(nk, flags, xBuf, sizeof(xBuf), nk_filter_decimal);
    ...
    nk_layout_row_push(nk, 1.f / 9.f);
    events = nk_edit_string_zero_terminated(nk, flags, yBuf, sizeof(yBuf), nk_filter_decimal);
    if (events & NK_EDIT_COMMITED)
    {
        y = atoi(yBuf);
        *isAutoUpdating = GLFW_FALSE;
    ...
    nk_layout_row_push(nk, 1.f / 9.f);
    events = nk_edit_string_zero_terminated(nk, flags, wBuf, sizeof(wBuf), nk_filter_decimal);
    if (events & NK_EDIT_COMMITED)
    {
        w = atoi(wBuf);
        *isAutoUpdating = GLFW_FALSE;
    ...
    nk_layout_row_push(nk, 1.f / 9.f);
    events = nk_edit_string_zero_terminated(nk, flags, hBuf, sizeof(hBuf), nk_filter_decimal);
    if (events & NK_EDIT_COMMITED)
    {
        h = atoi(hBuf);
        *isAutoUpdating = GLFW_FALSE;
