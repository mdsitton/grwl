...
GRWLbool _grwlSelectPlatform(int desiredID, _GRWLplatform* platform)
{
    ...
    if (desiredID == GRWL_PLATFORM_NULL)
    {
        return _grwlConnectNull(desiredID, platform);
    }
    else if (count == 0)
    {
        _grwlInputError(GRWL_PLATFORM_UNAVAILABLE, "This binary only supports the Null platform");
        return GRWL_FALSE;
    }
