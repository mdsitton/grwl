...
// Returns true when the converting completes and the result is assigned to
// the argument `codepoint`.
// Returns false when the converting is not yet completed (for
...
//
static bool convertToUTF32FromUTF16(WCHAR utf16_unit, WCHAR* highsurrogate, uint32_t* codepoint)
{
    *codepoint = 0;
...
static const GRWLimage* chooseImage(int count, const GRWLimage* images, int width, int height)
{
    ...
    const GRWLimage* closest = nullptr;
...
//
static HICON createIcon(const GRWLimage* image, int xhot, int yhot, bool icon)
{
    HDC dc;
    HICON handle;
    HBITMAP color, mask;
    ...
    mask = CreateBitmap(image->width, image->height, 1, 1, nullptr);
    if (!mask)
    {
...
{
    ClipCursor(nullptr);
    _grwl.win32.capturedCursorWindow = nullptr;
}
...
static void enableCursor(_GRWLwindow* window)
{
    ...
    _grwl.win32.disabledCursorWindow = nullptr;
    releaseCursor();
...
static void setCandidate(_GRWLpreeditcandidate* candidate, LPWSTR buffer)
{
    ...
    {
        unsigned int* text = (unsigned int*)_grwl_realloc(candidate->text, sizeof(unsigned int) * textBufferCount);
        if (text == nullptr)
        {
            return;
        }
    ...
    for (int i = 0; (size_t)i < bufferCount; ++i)
    {
...
static void getImmCandidates(_GRWLwindow* window)
{
    ...
    DWORD candidateListBytes = ImmGetCandidateListW(hIMC, 0, nullptr, 0);
...

    {
        ...
        LPCANDIDATELIST candidateList = (LPCANDIDATELIST)_grwl_calloc(candidateListBytes, 1);
        ...
        if (bufferCount != preedit->candidateBufferCount)
        {
            _GRWLpreeditcandidate* candidates =
                (_GRWLpreeditcandidate*)_grwl_realloc(preedit->candidates, sizeof(_GRWLpreeditcandidate) * bufferCount);
            ...
            for (int i = preedit->candidateBufferCount; i < bufferCount; ++i)
            {
                candidates[i].text = nullptr;
        ...
        for (int i = 0; (DWORD)i < candidateList->dwCount; ++i)
        {
...
//
static bool getImmPreedit(_GRWLwindow* window)
{
    _GRWLpreedit* preedit = &window->preedit;
    HIMC hIMC = ImmGetContext(window->win32.handle);
    // get preedit data sizes
    LONG preeditBytes = ImmGetCompositionStringW(hIMC, GCS_COMPSTR, nullptr, 0);
    LONG attrBytes = ImmGetCompositionStringW(hIMC, GCS_COMPATTR, nullptr, 0);
    LONG clauseBytes = ImmGetCompositionStringW(hIMC, GCS_COMPCLAUSE, nullptr, 0);
    ...
    if (preeditBytes > 0)
    {
        ...
        LPWSTR buffer = (LPWSTR)_grwl_calloc(preeditBytes, 1);
        LPSTR attributes = (LPSTR)_grwl_calloc(attrBytes, 1);
        DWORD* clauses = (DWORD*)_grwl_calloc(clauseBytes, 1);
        ...
        if (textBufferCount != preedit->textBufferCount)
        {
            ...
            unsigned int* preeditText = (unsigned int*)_grwl_realloc(preedit->text, bufsize);
        ...
        if (blockBufferCount != preedit->blockSizesBufferCount)
        {
            ...
            int* blocks = (int*)_grwl_realloc(preedit->blockSizes, bufsize);
...
//
static bool commitImmResultStr(_GRWLwindow* window)
{
    HIMC hIMC;
    LONG bytes;
    uint32_t codepoint;
    ...
    // get preedit data sizes
    bytes = ImmGetCompositionStringW(hIMC, GCS_RESULTSTR, nullptr, 0);

    if (bytes > 0)
    {
        int length = bytes / sizeof(WCHAR);
        LPWSTR buffer = (LPWSTR)_grwl_calloc(bytes, 1);
...
{
    _GRWLwindow* window = (_GRWLwindow*)GetPropW(hWnd, L"GRWL");
    if (!window)
    {
        if (uMsg == WM_NCCREATE)
        {
            if (_grwlIsWindows10Version1607OrGreaterWin32())
            {
                ...
                const _GRWLwndconfig* wndconfig = (_GRWLwndconfig*)cs->lpCreateParams;
    ...
    switch (uMsg)
    {
        ...
        case WM_UNICHAR:
        {
            ...
            _grwlInputChar(window, (uint32_t)wParam, getKeyMods(), true);
            return 0;
        }
        ...
        case WM_INPUT:
        {
            ...
            if (size > (UINT)_grwl.win32.rawInputSize)
            {
                ...
                _grwl.win32.rawInput = (RAWINPUT*)_grwl_calloc(size, 1);
        ...
        case WM_DROPFILES:
        {
            ...
            char** paths = (char**)_grwl_calloc(count, sizeof(char*));
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                WCHAR* buffer = (WCHAR*)_grwl_calloc((size_t)length + 1, sizeof(WCHAR));
    ...
    {
        HRESULT res = CoCreateInstance(CLSID_TaskbarList, nullptr, CLSCTX_INPROC_SERVER, IID_ITaskbarList3,
...
static int createNativeWindow(_GRWLwindow* window, const _GRWLwndconfig* wndconfig, const _GRWLfbconfig* fbconfig)
{
    ...
    if (!_grwl.win32.mainWindowClass)
    {
        ...
        wc.hIcon =
            (HICON)LoadImageW(GetModuleHandleW(nullptr), L"GRWL_ICON", IMAGE_ICON, 0, 0, LR_DEFAULTSIZE | LR_SHARED);
        if (!wc.hIcon)
        {
    ...
    {
        ChangeWindowMessageFilterEx(window->win32.handle, WM_DROPFILES, MSGFLT_ALLOW, nullptr);
        ChangeWindowMessageFilterEx(window->win32.handle, WM_COPYDATA, MSGFLT_ALLOW, nullptr);
        ChangeWindowMessageFilterEx(window->win32.handle, WM_COPYGLOBALDATA, MSGFLT_ALLOW, nullptr);

        window->win32.taskbarListMsgID = RegisterWindowMessageW(L"TaskbarButtonCreated");
        if (window->win32.taskbarListMsgID)
        {
            ChangeWindowMessageFilterEx(window->win32.handle, window->win32.taskbarListMsgID, MSGFLT_ALLOW, nullptr);
        }
    }
                            ...
                            const _GRWLfbconfig* fbconfig)
{
    ...
    {
        _grwlSetWindowMousePassthroughWin32(window, true);
    }
...
static HICON GenerateTextBadgeIcon(HWND hWnd, WCHAR* text)
{
    ...
    badgeData.hBitmap = CreateDIBSection(badgeData.hdc, &bmi, DIB_RGB_COLORS, &bits, nullptr, 0);
    ...
    badgeData.hOldBitmap = nullptr;
    SelectObject(badgeData.hdcMemMask, badgeData.hOldBitmapMask);
    badgeData.hOldBitmapMask = nullptr;
...
void _grwlSetWindowPosWin32(_GRWLwindow* window, int xpos, int ypos)
{
    ...
    SetWindowPos(window->win32.handle, nullptr, rect.left, rect.top, 0, 0, SWP_NOACTIVATE | SWP_NOZORDER | SWP_NOSIZE);
}
...
{
    HMONITOR handle = MonitorFromWindow(window->win32.handle, MONITOR_DEFAULTTONEAREST);
...
float _grwlGetWindowOpacityWin32(_GRWLwindow* window)
{
    ...
    if ((GetWindowLongW(window->win32.handle, GWL_EXSTYLE) & WS_EX_LAYERED) &&
        GetLayeredWindowAttributes(window->win32.handle, nullptr, &alpha, &flags))
    {
        if (flags & LWA_ALPHA)
        {
            return alpha / 255.f;
...
void _grwlPollEventsWin32()
{
    ...
    if (handle)
    {
        window = (_GRWLwindow*)GetPropW(handle, L"GRWL");
        if (window)
        {
...
{
    MsgWaitForMultipleObjects(0, nullptr, FALSE, (DWORD)(timeout * 1e3), QS_ALLEVENTS);
...
const char* _grwlGetKeyboardLayoutNameWin32()
{
    ...
    lcid = MAKELCID(LANGIDFROMLCID(wcstoul(klid, nullptr, 16)), 0);

    size = GetLocaleInfoW(lcid, LOCALE_SLANGUAGE, nullptr, 0);
    if (!size)
    {
    ...
    language = (WCHAR*)calloc(size, sizeof(WCHAR));
...
bool _grwlCreateStandardCursorWin32(_GRWLcursor* cursor, int shape)
{
    ...
    cursor->win32.handle =
        (HCURSOR)LoadImageW(nullptr, MAKEINTRESOURCEW(id), IMAGE_CURSOR, 0, 0, LR_DEFAULTSIZE | LR_SHARED);
    if (!cursor->win32.handle)
    {
...
void _grwlSetClipboardStringWin32(const char* string)
{
    ...
    characterCount = MultiByteToWideChar(CP_UTF8, 0, string, -1, nullptr, 0);
    if (!characterCount)
    {
        return;
    }
...
EGLenum _grwlGetEGLPlatformWin32(EGLint** attribs)
{
    if (_grwl.egl.ANGLE_platform_angle)
    {
        ...
        if (type)
        {
            *attribs = (EGLint*)_grwl_calloc(3, sizeof(EGLint));
