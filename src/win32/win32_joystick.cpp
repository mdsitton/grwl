...
{
    const _GRWLjoyobjectWin32* fo = (_GRWLjoyobjectWin32*)first;
    const _GRWLjoyobjectWin32* so = (_GRWLjoyobjectWin32*)second;
...
static bool supportsXInput(const GUID* guid)
{
    ...
    ridl = (RAWINPUTDEVICELIST*)_grwl_calloc(count, sizeof(RAWINPUTDEVICELIST));
...
{
    _GRWLobjenumWin32* data = (_GRWLobjenumWin32*)user;
...
static BOOL CALLBACK deviceCallback(const DIDEVICEINSTANCE* di, void* user)
{
    ...
    if (FAILED(IDirectInput8_CreateDevice(_grwl.win32.dinput8.api, di->guidInstance, &device, nullptr)))
    {
    ...
    data.device = device;
    data.objects =
        (_GRWLjoyobjectWin32*)_grwl_calloc(dc.dwAxes + (size_t)dc.dwButtons + dc.dwPOVs, sizeof(_GRWLjoyobjectWin32));
    ...
    if (!WideCharToMultiByte(CP_UTF8, 0, di->tszInstanceName, -1, name, sizeof(name), nullptr, nullptr))
    {
...
void _grwlDetectJoystickConnectionWin32()
{
    if (_grwl.win32.xinput.instance)
    {
        for (uint32_t index = 0; index < XUSER_MAX_COUNT; index++)
        {
            ...
            {
                if (_grwl.joysticks[jid].connected && _grwl.joysticks[jid].win32.device == nullptr &&
    ...
    {
        if (FAILED(IDirectInput8_EnumDevices(_grwl.win32.dinput8.api, DI8DEVCLASS_GAMECTRL, deviceCallback, nullptr,
                                             DIEDFL_ALLDEVICES)))
        {
...
bool _grwlInitJoysticksWin32()
{
    if (_grwl.win32.dinput8.instance)
    {
        if (FAILED(DirectInput8Create(_grwl.win32.instance, DIRECTINPUT_VERSION, IID_IDirectInput8W,
                                      (void**)&_grwl.win32.dinput8.api, nullptr)))
        {
...
{
    for (int jid = GRWL_JOYSTICK_1; jid <= GRWL_JOYSTICK_LAST; jid++)
    {
...
bool _grwlPollJoystickWin32(_GRWLjoystick* js, int mode)
{
    if (js->win32.device)
    {
        ...
        for (int i = 0; i < js->win32.objectCount; i++)
        {
    ...
    else
    {
        int dpad = 0;
        XINPUT_STATE xis;
        const uint16_t buttons[10] = { XINPUT_GAMEPAD_A,
                                       XINPUT_GAMEPAD_B,
                                       XINPUT_GAMEPAD_X,
                                       XINPUT_GAMEPAD_Y,
        ...
        uint32_t result = XInputGetState(js->win32.index, &xis);
