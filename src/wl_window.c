...
static struct wl_buffer* createShmBuffer(const GLFWimage* image)
{
    ...
    if (fd < 0)
    {
        _glfwInputError(GLFW_PLATFORM_ERROR, "Wayland: Failed to create buffer file of size %d: %s", length,
                        strerror(errno));
        return NULL;
    }
...
static void createFallbackDecoration(_GLFWwindow* window, _GLFWdecorationWayland* decoration, struct wl_surface* parent,
...
static void createFallbackDecorations(_GLFWwindow* window)
{
    ...
    createFallbackDecoration(window, &window->wl.decorations.top, window->wl.surface, window->wl.decorations.buffer, 0,
                             -GLFW_CAPTION_HEIGHT, window->wl.width, GLFW_CAPTION_HEIGHT);
    createFallbackDecoration(window, &window->wl.decorations.left, window->wl.surface, window->wl.decorations.buffer,
                             -GLFW_BORDER_SIZE, -GLFW_CAPTION_HEIGHT, GLFW_BORDER_SIZE,
    ...
    createFallbackDecoration(window, &window->wl.decorations.bottom, window->wl.surface, window->wl.decorations.buffer,
                             -GLFW_BORDER_SIZE, window->wl.height, window->wl.width + GLFW_BORDER_SIZE * 2,
                             GLFW_BORDER_SIZE);
}
...
static void resizeWindow(_GLFWwindow* window)
{
    ...
    wp_viewport_set_destination(window->wl.decorations.top.viewport, window->wl.width, GLFW_CAPTION_HEIGHT);
    wl_surface_commit(window->wl.decorations.top.surface);

    wp_viewport_set_destination(window->wl.decorations.left.viewport, GLFW_BORDER_SIZE,
                                window->wl.height + GLFW_CAPTION_HEIGHT);
    wl_surface_commit(window->wl.decorations.left.surface);

    wl_subsurface_set_position(window->wl.decorations.right.subsurface, window->wl.width, -GLFW_CAPTION_HEIGHT);
    wp_viewport_set_destination(window->wl.decorations.right.viewport, GLFW_BORDER_SIZE,
                                window->wl.height + GLFW_CAPTION_HEIGHT);
    wl_surface_commit(window->wl.decorations.right.surface);

    wl_subsurface_set_position(window->wl.decorations.bottom.subsurface, -GLFW_BORDER_SIZE, window->wl.height);
    wp_viewport_set_destination(window->wl.decorations.bottom.viewport, window->wl.width + GLFW_BORDER_SIZE * 2,
                                GLFW_BORDER_SIZE);
...
static void surfaceHandleEnter(void* userData, struct wl_surface* surface, struct wl_output* output)
{
    ...
    {
        window->wl.scaleSize++;
        window->wl.scales = _glfw_realloc(window->wl.scales, window->wl.scaleSize * sizeof(_GLFWscaleWayland));
    }
...
static GLFWbool createNativeSurface(_GLFWwindow* window, const _GLFWwndconfig* wndconfig, const _GLFWfbconfig* fbconfig)
{
...
static char* readDataOfferAsString(struct wl_data_offer* offer, const char* mimeType)
{
    ...
    {
        _glfwInputError(GLFW_PLATFORM_ERROR, "Wayland: Failed to create pipe for data offer: %s", strerror(errno));
        return NULL;
    }
    ...
    for (;;)
    {
        ...
        else if (result == -1)
        {
            ...
            _glfwInputError(GLFW_PLATFORM_ERROR, "Wayland: Failed to read from data offer pipe: %s", strerror(errno));
            close(fds[0]);
            return NULL;
        }
                                ...
                                uint32_t button, uint32_t state)
{
    ...
    {
        if (window->wl.decorations.focus != GLFW_MAIN_WINDOW && window->wl.xdg.toplevel)
        {
                                    ...
                                    uint32_t modsDepressed, uint32_t modsLatched, uint32_t modsLocked, uint32_t group)
{
    ...
    struct
    {
        ...
        { _glfw.wl.xkb.controlIndex, GLFW_MOD_CONTROL },    { _glfw.wl.xkb.altIndex, GLFW_MOD_ALT },
        { _glfw.wl.xkb.shiftIndex, GLFW_MOD_SHIFT },        { _glfw.wl.xkb.superIndex, GLFW_MOD_SUPER },
        { _glfw.wl.xkb.capsLockIndex, GLFW_MOD_CAPS_LOCK }, { _glfw.wl.xkb.numLockIndex, GLFW_MOD_NUM_LOCK }
    };

    for (size_t i = 0; i < sizeof(modifiers) / sizeof(modifiers[0]); i++)
    {
        if (xkb_state_mod_index_is_active(_glfw.wl.xkb.state, modifiers[i].index, XKB_STATE_MODS_EFFECTIVE) == 1)
        {
...
{
    _GLFWofferWayland* offers = _glfw_realloc(_glfw.wl.offers, sizeof(_GLFWofferWayland) * (_glfw.wl.offerCount + 1));
    if (!offers)
    {
        _glfwInputError(GLFW_OUT_OF_MEMORY, NULL);
        return;
                                     ...
                                     int32_t cursorBegin, int32_t cursorEnd)
{
    ...
    while (cur && *cur)
    {
        ...
        if (preedit->textBufferCount < preedit->textCount + 1)
        {
            ...
            preedit->text = _glfw_realloc(preedit->text, sizeof(unsigned int) * bufSize);
            if (!preedit->text)
            {
                return;
            }
                              ...
                              uint32_t sym, uint32_t state, uint32_t modifiers)
{
    ...
    keyboardHandleKey(data, _glfw.wl.keyboard, serial, time, scancode, state);
}
...
GLFWbool _glfwCreateWindowWayland(_GLFWwindow* window, const _GLFWwndconfig* wndconfig, const _GLFWctxconfig* ctxconfig,
                                  const _GLFWfbconfig* fbconfig)
{
    ...
    {
        window->wl.textInputV3 = zwp_text_input_manager_v3_get_text_input(_glfw.wl.textInputManagerV3, _glfw.wl.seat);
        zwp_text_input_v3_add_listener(window->wl.textInputV3, &textInputV3Listener, window);
    }
    else if (_glfw.wl.textInputManagerV1)
    {
        window->wl.textInputV1 = zwp_text_input_manager_v1_create_text_input(_glfw.wl.textInputManagerV1);
...
const char* _glfwGetKeyboardLayoutNameWayland(void)
{
    if (!_glfw.wl.xkb.keymap)
    {
        _glfwInputError(GLFW_PLATFORM_ERROR, "Wayland: Keymap missing");
        return NULL;
    }

    const char* name = xkb_keymap_layout_get_name(_glfw.wl.xkb.keymap, _glfw.wl.xkb.group);
    if (!name)
    {
        _glfwInputError(GLFW_PLATFORM_ERROR, "Wayland: Failed to query keyboard layout name");
        return NULL;
    }
...
GLFWbool _glfwCreateStandardCursorWayland(_GLFWcursor* cursor, int shape)
{
    ...
    {
        _glfwInputError(GLFW_CURSOR_UNAVAILABLE, "Wayland: No cursor theme");
        return GLFW_FALSE;
    }
    ...
    if (!cursor->wl.cursor)
    {
        ...
        if (!cursor->wl.cursor)
        {
            _glfwInputError(GLFW_CURSOR_UNAVAILABLE, "Wayland: Failed to create standard cursor \"%s\"", name);
            return GLFW_FALSE;
        }
...
static void lockPointer(_GLFWwindow* window)
{
    ...
    window->wl.lockedPointer = zwp_pointer_constraints_v1_lock_pointer(_glfw.wl.pointerConstraints, window->wl.surface,
                                                                       _glfw.wl.pointer, NULL,
...
static void confinePointer(_GLFWwindow* window)
{
    window->wl.confinedPointer =
        zwp_pointer_constraints_v1_confine_pointer(_glfw.wl.pointerConstraints, window->wl.surface, _glfw.wl.pointer,
...
static void dataSourceHandleSend(void* userData, struct wl_data_source* source, const char* mimeType, int fd)
{
    ...
    while (length > 0)
    {
        ...
        if (result == -1)
        {
            ...
            _glfwInputError(GLFW_PLATFORM_ERROR, "Wayland: Error while writing the clipboard: %s", strerror(errno));
            break;
        }
...
GLFWbool _glfwGetPhysicalDevicePresentationSupportWayland(VkInstance instance, VkPhysicalDevice device,
