...
static GLFWbool createEmptyEventPipe(void)
{
    ...
    {
        _glfwInputError(GLFW_PLATFORM_ERROR, "X11: Failed to create empty event pipe: %s", strerror(errno));
        return GLFW_FALSE;
    }
    ...
    for (int i = 0; i < 2; i++)
    {
        ...
        if (sf == -1 || df == -1 || fcntl(_glfw.x11.emptyEventPipe[i], F_SETFL, sf | O_NONBLOCK) == -1 ||
            fcntl(_glfw.x11.emptyEventPipe[i], F_SETFD, df | FD_CLOEXEC) == -1)
        {
            _glfwInputError(GLFW_PLATFORM_ERROR, "X11: Failed to set flags for empty event pipe: %s", strerror(errno));
            return GLFW_FALSE;
        }
    }
...
int _glfwInitX11(void)
{
    ...
    _glfw.x11.xlib.GetAtomName = (PFN_XGetAtomName)_glfwPlatformGetModuleSymbol(_glfw.x11.xlib.handle, "XGetAtomName");
    ...
    _glfw.x11.xlib.SetICValues = (PFN_XSetICValues)_glfwPlatformGetModuleSymbol(_glfw.x11.xlib.handle, "XSetICValues");
    ...
    _glfw.x11.xlib.VaCreateNestedList =
        (PFN_XVaCreateNestedList)_glfwPlatformGetModuleSymbol(_glfw.x11.xlib.handle, "XVaCreateNestedList");
    ...
    _glfw.x11.xkb.AllocKeyboard =
        (PFN_XkbAllocKeyboard)_glfwPlatformGetModuleSymbol(_glfw.x11.xlib.handle, "XkbAllocKeyboard");
    ...
    _glfw.x11.xlib.mbResetIC = (PFN_XmbResetIC)_glfwPlatformGetModuleSymbol(_glfw.x11.xlib.handle, "XmbResetIC");
