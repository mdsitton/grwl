...
{
    angle = 100.f * (float)grwlGetTime();
}

/* change view angle, exit upon ESC */
void key(GRWLwindow* window, int k, int s, int action, int mods)
{
...
/* new window size */
void reshape(GRWLwindow* window, int width, int height)
{
...
int main(int argc, char* argv[])
{
    ...
    grwlWindowHint(GRWL_DEPTH_BITS, 16);
    grwlWindowHint(GRWL_TRANSPARENT_FRAMEBUFFER, GRWL_TRUE);

    window = grwlCreateWindow(300, 300, "Gears", NULL, NULL);
    if (!window)
    {
