 ...
 *          GRWL conversion by Marcus Geelnard
...
void init(void);
void display(void);
void reshape(GRWLwindow* window, int w, int h);
...
void key_callback(GRWLwindow* window, int key, int scancode, int action, int mods)
{
    ...
    if (key == GRWL_KEY_ESCAPE && mods == 0)
    {
        grwlSetWindowShouldClose(window, GRWL_TRUE);
    }
    if ((key == GRWL_KEY_ENTER && mods == GRWL_MOD_ALT) || (key == GRWL_KEY_F11 && mods == GRWL_MOD_ALT))
    {
...
void mouse_button_callback(GRWLwindow* window, int button, int action, int mods)
{
    ...
    if (action == GRWL_PRESS)
    {
        override_pos = GRWL_TRUE;
        set_ball_pos(cursor_x, cursor_y);
    }
    else
    {
        override_pos = GRWL_FALSE;
    }
}
...
int main(void)
{
    ...
    window = grwlCreateWindow(400, 400, "Boing (classic Amiga demo)", NULL, NULL);
    if (!window)
    {
        grwlTerminate();
        exit(EXIT_FAILURE);
    ...
    grwlSetWindowAspectRatio(window, 1, 1);

    grwlSetFramebufferSizeCallback(window, reshape);
